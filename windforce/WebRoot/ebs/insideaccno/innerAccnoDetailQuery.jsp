...
<script type="text/javascript">
	...
	function viewAccnoDetailListByPage(pageNumber){
		...
		}else if(document.getElementById("traceDate").value.length == 0){
			alert("交易日期不能为空！");
			document.getElementById("traceDate").focus();
		}else{
			if(IsDate(document.getElementById("traceDate"))){
				$("#curPage").val(pageNumber);
				$("#innerAccnoDetailQueryForm").submit();
			}else{
				document.getElementById("traceDate").focus();
			}
		}
	...
	function toExportData() {
		var objTable = document.getElementById("accnoDetailQueryInfo");	
		if (objTable.rows.length>2) {
			window.location.href = "innerAccnoDetailQuery_exportData.action";
		}else{
			alert("查询结果列表无数据");
		}
	}
...
<body class="baby_in2">
	<input id="moudleName" type="hidden" value="内部账户发生额明细查询" />
	<form id="innerAccnoDetailQueryForm" method="post"
		action="innerAccnoDetailQuery_queryAccnoDetailData.action" name="innerAccnoDetailQuery">
		<div class="nov_moon">
					...
					<tr>
						<td width="20%">交易账号&nbsp;
							<input name="queryParam.accno" type="text"
								...
								maxlength="32" value='<s:property value="queryParam.accno"/>' />
						</td>
						<td width="20%">交易日期&nbsp;
							<input name="queryParam.traceDate" type="text"
								class="diinput_text01" id="traceDate" title='交易日期'
								onclick="new Calendar().show(this);" maxlength="10"
								value='<s:property value="queryParam.traceDate"/>' />
							<input type="hidden" id="curPage" name="queryParam.curPage"
								class="diinput_text01" value='<s:property value="queryParam.curPage"/>' />
						</td>
			...
			<div style="width:100%; height:auto; float:left; margin-left:10px; padding:10px 0px 10px 0px; color: #000000;">
				...
				<table width="98%" border="0" cellpadding="0" cellspacing="0"
					id="accnoDetailQueryInfo">
					<tr>
						<td width="5%" height="26" align="center" bgcolor="#c76c6f" class="font_colors01">序号</td>
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">交易日期</td>
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">交易账号</td>
						...
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">对方户名</td>
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">发生额</td>
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">实时余额</td>
						<td width="10%" align="center" bgcolor="#c76c6f" class="font_colors01">交易时间</td>
						<td width="13%" align="center" bgcolor="#c76c6f" class="font_colors01">摘要</td>
					...
					<tbody id="queryList" align="center">
						<s:iterator value="queryList" var="queryData" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#queryData.trad_date" /></td>
								<td><s:property value="#queryData.acct" /></td>
								<td><s:property value="refDcFlagMap.get(#queryData.borrow_lend_sign)" /></td>
								<td><s:property value="#queryData.oppost_acct" /></td>
								<td><s:property value="#queryData.oppost_acct_name" /></td>
								<td><s:property value="#queryData.trad_amt" /></td>
								<td><s:property value="#queryData.acct_bal" /></td>
								<td><s:property value="#queryData.host_syst_time" /></td>
								<td><s:property value="#queryData.summy" /></td>
							</tr>
						</s:iterator>
					</tbody>
					...
					<tfoot id="accnoDetailListPageInfo">
						<tr class="pagelinks">
							<td colspan="4">
								...
								到
								<s:property value="queryParam.lastResult" />
								条，共
								<s:property value="queryParam.total" />
								条
							</td>
							<td colspan="8" align="right">
								...
								<s:else>
									<a href="#" onclick="viewAccnoDetailListByPage('1');return false;">首页</a>&nbsp;&nbsp;<a
										href="#"
										onclick="viewAccnoDetailListByPage('${queryParam.curPage - 1}');return false;">上一页</a>
								</s:else>
								<s:if
									test="queryParam.curPage==queryParam.totalPage">
									<a>下一页</a>&nbsp;&nbsp;<a>尾页</a>
								</s:if>
								<s:else>
									<a href="#"
										onclick="viewAccnoDetailListByPage(${queryParam.curPage + 1});return false;">下一页</a>&nbsp;&nbsp;<a
										href="#"
										onclick="viewAccnoDetailListByPage('${queryParam.totalPage}');return false;">尾页</a>
								</s:else>
							</td>
						</tr>
					</tfoot>
