	...
	if(isValid) {
		...
		for(var i = 0 ; i < tags.length;i++) {
			...
			if(tags[i].checkType == "01") {
				isOnly = false;
				//查数据库，进行比对
				$.post("ParamsManagerAction_getDataforIsiterance.action","opeateType=1&checkTableName="+reportName+"&checkKey="+tags[i].id+"&checkValue="+tags[i].value,function(data){
					if(data!=0){
						isValid = false;
						alert(data);
					...
					if(index == tags.length)
						{
						 if(isValid)
						 {
						 $.post("ParamsManagerAction_updateParams.action","queryForm="+queryString+"&reportName="+reportName, function(data){
							 alert(data);
