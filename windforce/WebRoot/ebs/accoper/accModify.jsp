...
<script type="text/javascript">
	...
	function toExportData() {
		...
		if (objTable.rows.length>2) {
			window.location.href = "accModifyAction_exportData.action";
		}else{
			alert("查询结果无数据");
		}
	}	
	...
	$(document).ready(function(){
		...
		$("#sealMode").append("<option value='5'>无</option>");
		var sealMode = "<s:property value='acountQueryParam.sealMode'/>";
		if(5==sealMode){
			document.getElementById("sealMode").options[3].selected=true;
		}
	});
</script>
<style>
...
<body class="baby_in2">
	...
	<input id="moudleName" type="hidden" value="账户信息维护"/>
	<div id="son_div" class="son_div">
		...
		<jsp:include page="/ebs/accoper/accmodify_div.jsp" />
	</div>
	
	<div id="nov_nr" class="nov_nr">
		<form id="accModifyForm"
					action="accModifyAction_queryAccData.action" name="accModify"
					method="post">
		<div class="nov_moon">
			<div
				...
				class="border_bottom01">
					<table width="98%">
						<tr>
							...
							<td width="20%">账户地址&nbsp;&nbsp; 
								<input name="acountQueryParam.address" type="text"
									class="diinput_text01" id="txtaddress" maxlength="256"
									value='<s:property value="acountQueryParam.address"/>' /> 	
						...
						<tr>
							...
							<td width="20%">验印模式&nbsp;&nbsp; 
								<s:select list="sealModeMap" listKey="key" style="width:165px" 
									listValue="value" headerKey="" headerValue="--请选择--" value="acountQueryParam.sealMode"
									name="acountQueryParam.sealMode" id="sealMode">
								</s:select>
							</td>
							<td width="20%">发送方式&nbsp;&nbsp; 
								<s:select list="sendTypeMap" listKey="key" style="width:165px" 
