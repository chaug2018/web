...
<script type="text/javascript">
	...
	function toExportData() {
		var objTable = document.getElementById("rushStatisticsList");	
		if (objTable.rows.length>2) {
			window.location.href = "rushStatisticsAction_exportData.action";
		}else{
			alert("统计结果列表无数据");
		}
	}
...
<body class="baby_in2">
	...
	<input id="moudleName" type="hidden" value="催收情况统计"/>
	<form id="rushStatisticsForm" action="rushStatisticsAction_getResult.action"
		name="rushStatistics" method="post">
		<div class="nov_moon">
			<div style="width:100%; height:auto; float:left; margin-left:10px; 
				padding:10px 0px 10px 0px;"	class="border_bottom01">
               <input type="hidden" name="rushStatisticsParam.curPage" id="curPage" />
				<table width="98%">
					<tr>
						<td width="20%">总&nbsp;&nbsp;&nbsp;&nbsp; 行&nbsp;&nbsp;&nbsp;&nbsp;
							<select>
					 	...
					 	<td width="20%">机 构 号&nbsp;
					   		 <input	name="rushStatisticsParam.idBank1" type="text" 
								class="diinput_text01" id="idBank" 
								value='<s:property value="rushStatisticsParam.idBank1"/>' />
						</td>
					...
					<tr>
						<td width="20%">分&nbsp;&nbsp;&nbsp;&nbsp;行&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<select	name="rushStatisticsParam.idCenter" id="selectIdCenter" 
								 onchange="changeIdCenter()" onclick="judgeCountIdBranch()" >
							</select>
							</td>
						<td width="20%">对账日期 
							<input  title="对账日期"	name="rushStatisticsParam.docDate" type="text"
							class="input_text01" id="docdate" onclick="new Calendar().show(this);" 
							value='<s:property value="rushStatisticsParam.docDate"/>'/>
						</td>
						<td width="10%" align="center">
					...
					<tr>
						<td width="20%">网 &nbsp;&nbsp;&nbsp;&nbsp;点&nbsp;&nbsp;&nbsp;&nbsp;
							<select name="rushStatisticsParam.idBank" id="selectIdBank"  
			...
			<div style="width:98%; height:auto; float:left; margin-left:10px; padding:10px 0px 10px 0px;">
				<h1>催收情况统计</h1>
				<table width="100%" border="0" cellpadding="0" cellspacing="0" id="rushStatisticsList">
					<thead>
						<tr>
							...
							<td width="12%" align="center" bgcolor="#c76c6f"
								class="font_colors01">对账<br/>日期</td>
							<td width="7%" height="26" align="center" bgcolor="#c76c6f"
								class="font_colors01">对账单<br/>发出数</td>
							<td width="7%" align="center" bgcolor="#c76c6f"
								class="font_colors01">应催<br/>收总数</td>
							<td width="7%" align="center" bgcolor="#c76c6f"
								class="font_colors01">催收<br/>成功数</td>
							<td width="7%" align="center" bgcolor="#c76c6f"
								class="font_colors01">电话<br/>催收数</td>
							<td width="7%" align="center" bgcolor="#c76c6f"
								class="font_colors01">邮件<br/>催收数</td>
							<td width="7%" align="center" bgcolor="#c76c6f"
								class="font_colors01">面对面<br/>催收数</td>
							<td width="10%" align="center" bgcolor="#c76c6f"
								class="font_colors01">催收<br/>成功率</td>
						</tr>
					</thead>
					...
					<tbody id="queryList" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr <s:if test="#st.index%2 ==0">bgcolor="#CBD6E0"</s:if>
								...
								<td><s:property	value="#st.count+rushStatisticsParam.firstResult" /></td>
								...
								<td><s:property value="#result.totalAmount" /></td>
								<td><s:property value="#result.rushedAmount" /></td>
								<td><s:property value="#result.successAmount" /></td>
								<td><s:property value="#result.telAmount" /></td>
								<td><s:property value="#result.emailAmount" /></td>
								<td><s:property value="#result.faceAmount" /></td>
								<td><s:property value="#result.successRate" /></td>
							</tr>
						</s:iterator>
					</tbody>
					<tfoot id="rushStatisticsFoot">
						<tr class="pagelinks">
							<td colspan="7">每页显示 
								<select name="rushStatisticsParam.pageSize"	id="pageSize" style="width:40px;"
									onchange="viewStatisticsDetail('1')">
									<option value="10" <s:if test="rushStatisticsParam.pageSize==10">selected="selected"</s:if>>
										10
									</option>
									<option value="20" <s:if test="rushStatisticsParam.pageSize==20">selected="selected"</s:if>>
										20
									</option>
									<option value="50" <s:if test="rushStatisticsParam.pageSize==50">selected="selected"</s:if>>
										50
									</option>
								</select>条记录&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当前显示
								<s:if test="rushStatisticsParam.total==0">
									0
								</s:if> 
								<s:else>
									<s:property value="rushStatisticsParam.firstResult+1" />
								</s:else>到
								<s:property value="rushStatisticsParam.lastResult" />条，共
								<s:property	value="rushStatisticsParam.total" />条
							</td>
							<td colspan="8" align="right">
								<s:if test="rushStatisticsParam.curPage==1">
									<a>首页</a>&nbsp;&nbsp;
									<a>上一页</a>
								</s:if> 
								<s:else>
									<a href="#" onclick="viewStatisticsDetail('1');">首页</a>&nbsp;&nbsp;
									<a href="#"	onclick="viewStatisticsDetail('${rushStatisticsParam.curPage - 1}');return false;">上一页</a>
								</s:else> 
								<s:if test="rushStatisticsParam.curPage==rushStatisticsParam.totalPage">
									<a>下一页</a>&nbsp;&nbsp;
									<a>尾页</a>
								</s:if> 
								<s:else>
									<a href="#"	onclick="viewStatisticsDetail('${rushStatisticsParam.curPage + 1}');return false;">下一页</a>&nbsp;&nbsp;
									<a href="#"	onclick="viewStatisticsDetail('${rushStatisticsParam.totalPage}');return false;">尾页</a>
								</s:else>
							</td>
						</tr>
					</tfoot>
