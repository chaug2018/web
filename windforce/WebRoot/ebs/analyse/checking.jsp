...
<script type="text/javascript">
	...
	function toExportData() {
		...
		if (objTable.rows.length > 2) {
			window.location.href = 'ebillAnalyse_exportData.action';
		} else {
			alert("统计列表无数据");
		}
	}
	...
	$(document).ready(
			function() {
				...
				stopProcess();
				initTree(
				<%=request.getAttribute("orgTree")%>, idCenter, idBank);
	});
</script>
</head>
...
<body class="baby_in2">
	...
	<form id="ebillAnalyseForm" action="ebillAnalyse_analyse.action"
					...
					<thead>
						<tr>
							...
							<td width="5%" align="center" bgcolor="#c76c6f"
								class="font_colors01">退信的账户数</td>
							<td width="5%" align="center" bgcolor="#c76c6f"
								class="font_colors01">验印成功账户数</td>
							<td width="5%" align="center" bgcolor="#c76c6f"
								class="font_colors01">验印不符账户数</td>
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">验印成功率</td>
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">验印不符率</td>
							<td width="5%" align="center" bgcolor="#c76c6f"
								class="font_colors01">未建库账户数</td>
							<td width="5%" align="center" bgcolor="#c76c6f"
					...
					<tbody id="analyse" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#result.retreatCount" /></td>
								<td><s:property value="#result.proveMatchCount" /></td>
								<td><s:property value="#result.proveNotMatchCount" /></td>
								<td><s:property value="#result.proveMatchPercent" /></td>
								<td><s:property value="#result.proveNotMatchPercent" /></td>
								<td><s:property value="#result.wjkCount" /></td>
