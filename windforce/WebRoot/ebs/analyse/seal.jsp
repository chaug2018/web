...
<script type="text/javascript">
	function toExportData() {
		...
		if (objTable.rows.length > 2) {
			window.location.href = 'sealAnalyse_exportData.action';
		} else {
			alert("统计列表无数据");
		}
	}
...
<body class="baby_in2">
	<input id="moudleName" type="hidden" value="验印情况统计"/>
	<%@ include file="/ebs/taskviewtool/processing_div.jsp"%>
	<script type="text/javascript">startProcess("正在初始化界面。。。");</script>
	<form id="sealAnalyseForm" action="sealAnalyse_analyse.action" name="sealAnalyse" method="post">
		<div class="nov_moon">
			<div style="width:100%; height:auto; float:left; margin-left:10px; padding:10px 0px 10px 0px;"
				...
				<table width="98%">
					...
					<tr>
						...
						<td width="20%">对账日期&nbsp;&nbsp;&nbsp; 
							<input title="对账日期" name="queryParam.docDate" 
							id="docDate" class="diinput_text01"  maxlength="10"
					...
					<tbody id="analyse" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#result.proveMatchCount" /></td>
								<td><s:property value="#result.proveNotMatchCount" /></td>
								<td><s:property value="#result.notProve" /></td>
								<td><s:property value="#result.provePercent" /></td>
							</tr>
						</s:iterator>
					</tbody>
					<tfoot id="sealFoot">
