...
function deleteAcc(data){
	...
	if(confirm("确认删除吗?")){
		startProcess("正在删除...");
		$.post("ischeckModifyAction_delete.action", 
...
function modifyAcc(data){
	...
	$.getJSON("ischeckModifyAction_getBasicInfoByIndex.action", 
...
function submitModify(){
	...
	var parten = /^\s*$/ ; 
	if(isCheck=="1"){
		if(parten.test(remark)||remark==""){
			alert("请输入维护原因！");
			document.getElementById("remark_div").focus();//把光标移到维护原因输入框
		}else{
			if(confirm("确认提交修改吗?")){
				startProcess("正在修改...");
				$.post("ischeckModifyAction_modify.action", 
						param,
						function(result){
