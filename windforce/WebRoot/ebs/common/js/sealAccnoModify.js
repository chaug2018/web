...
function deleteAcc(data){
	...
	if(confirm("确认删除吗?")){
		startProcess("正在删除...");
		$.post("sealAccnoModifyAction_delete.action", 
...
function modifyAcc(data){
	...
	$.getJSON("sealAccnoModifyAction_getBasicInfoByIndex.action", 
			...
			function(basicInfo) {
				if(basicInfo==null){
					...
					document.getElementById("remark_div").value=basicInfo.remark;
...
function submitModify(){
	...
	param=param+"&remark="+document.getElementById("remark_div").value;
	var isCheck = document.getElementById("isCheck_div").value;
	var remark = document.getElementById("remark_div").value;
	var sealAccno = document.getElementById("sealNo_div").value;
	//判断验印账号是否为空
	var parten = /^\s*$/ ; 
	if(parten.test(sealAccno)||sealAccno==""){
		alert("请输入验印账号！");
		document.getElementById("sealNo_div").focus();//把光标移到维护原因输入框
	}else{
		if(confirm("确认提交修改吗?")){
			startProcess("正在修改...");
			$.post("sealAccnoModifyAction_modify.action", 
					param,
					function(result){
...
function changeRemark(){
	var remark = document.getElementById("remark_div");
	remark.value = "";
}