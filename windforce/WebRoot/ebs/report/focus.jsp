...
<script type="text/javascript">
	...
	function toExportData() {
		...
		if (objTable.rows.length > 2) {
			window.location.href = 'focusReport_exportData.action';
		} else {
			alert("统计列表无数据");
		}
	}
...
<body class="baby_in2">
	<input id="moudleName" type="hidden" value="对账集中情况统计" />
	<%@ include file="/ebs/taskviewtool/processing_div.jsp"%>
	<script type="text/javascript">startProcess("正在初始化界面。。。");</script>
	<form id="focusReportForm" action="focusReport_analyse.action"
		name="focusReport" method="post">
		<div class="nov_moon">
					...
					<thead>
						<tr>
							...
							<td width="6%" height="26" align="center" bgcolor="#c76c6f"
								class="font_colors01">邮寄对账账户数</td>
							<td width="6%" height="26" align="center" bgcolor="#c76c6f"
							...
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">其它对账账户数</td>
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">需对账账户数</td>
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">集中度</td>
					...
					<tbody id="analyse" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#result.mailCount" /></td>
								<td><s:property value="#result.netCount" /></td>
								<td><s:property value="#result.faceCount" /></td>
								<td><s:property value="#result.counterCount" /></td>
								<td><s:property value="#result.otherCount" /></td>
								<td><s:property value="#result.checkCount" /></td>
								<td><s:property value="#result.focusPercent" /></td>
							</tr>
						</s:iterator>
					</tbody>
					<tfoot id="ebillFoot">
