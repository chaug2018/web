...
<script type="text/javascript">
	...
	function toExportData() {
		...
		if (objTable.rows.length > 2) {
			window.location.href = 'netCheckReport_exportData.action';
		} else {
			alert("统计列表无数据");
		}
	}
...
<body class="baby_in2">
	<input id="moudleName" type="hidden" value="网银对账率统计" />
	<%@ include file="/ebs/taskviewtool/processing_div.jsp"%>
	<script type="text/javascript">startProcess("正在初始化界面。。。");</script>
	<form id="netCheckReportForm" action="netCheckReport_analyse.action"
		name="netCheckReport" method="post">
		<div class="nov_moon">
					...
					<thead>
						<tr>
							...
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">网银对账成功数</td>
							<td width="6%" align="center" bgcolor="#c76c6f"
								class="font_colors01">网银对账率</td>
					...
					<tbody id="analyse" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#result.netCheckSuccessCount" /></td>
								<td><s:property value="#result.netCheckSuccessPercent" /></td>
							</tr>
						</s:iterator>
					</tbody>
					<tfoot id="ebillFoot">
