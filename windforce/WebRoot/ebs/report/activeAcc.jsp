...
<script type="text/javascript">
	...
	function toExportData() {
		...
		if (objTable.rows.length > 2) {
			window.location.href = 'activeAccReport_exportData.action';
		} else {
			alert("统计列表无数据");
		}
	}
	...
	function viewAnalyseListByPage(pageNumber) {
		if (document.getElementById("workDate").value.length == 0) {
			alert("交易日期不能为空！");
			document.getElementById("workDate").focus();
		}
		else {
			$("#curPage").val(pageNumber);
			$("#activeAccReportForm").submit();
		}
	}
	...
	$(document).ready(function() {
	    ...
	    $("[inputNumber='number']").keypress(function(){
			//0-9:48-57, -:45  .:46  enter:13,只能输入48-57和13
...
<body class="baby_in2">
	<input id="moudleName" type="hidden" value="活跃账户情况统计" />
	<%@ include file="/ebs/taskviewtool/processing_div.jsp"%>
	<script type="text/javascript">startProcess("正在初始化界面。。。");</script>
	<form id="activeAccReportForm" action="activeAccReport_analyse.action"
		name="activeAccReport" method="post">
		<div class="nov_moon">
			<div style="width:100%; height:auto; float:left; margin-left:10px; padding:10px 0px 10px 0px;"
				...
				<table width="98%">
					<tr>
						...
						<td width="20%">机&nbsp;构&nbsp;&nbsp;号&nbsp;&nbsp;&nbsp;
							<input name="queryParam.idBank1" id="idBank" class="diinput_text01" 
								inputNumber='number' maxlength="8" value='<s:property value="queryParam.idBank1"/>' />
						</td>
						<td width="20%">发生额明细开始次数&nbsp;&nbsp;&nbsp;&nbsp;
							<input name="queryParam.beginNum" id="beginNum" class="diinput_text01" 
								inputNumber='number' maxlength="8" value='<s:property value="queryParam.beginNum"/>' />
						</td>
					...
					<tr>
						...
						<td width="20%">交易日期&nbsp;&nbsp;&nbsp; 
							<input title="交易日期" name="queryParam.workDate" 
							id="workDate" class="diinput_text01"  maxlength="10"
							value='<s:property value="queryParam.workDate"/>'
							onclick="new Calendar().show(this);" />
						</td>
						<td width="20%">发生额明细结束次数&nbsp;&nbsp;&nbsp;&nbsp;
							<input name="queryParam.endNum" id="endNum" class="diinput_text01" 
								inputNumber='number' maxlength="8" value='<s:property value="queryParam.endNum"/>' />
						</td>
					...
					<tbody id="analyse" align="center">
						<s:iterator value="resultList" var="result" status="st">
							<tr bgcolor='<s:if test="#st.count%2==0">#F4DDDF</s:if>'>
								...
								<td><s:property value="#result.creditCount" /></td>
							</tr>
						</s:iterator>
					</tbody>
					<tfoot id="ebillFoot">
