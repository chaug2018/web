 ...
 * @version 1.0.0
 */
public class EbillMatchReportBizImpl implements IEbillMatchReportBiz{
	
	private static final java.text.DecimalFormat df = new java.text.DecimalFormat("#.##");

	private EbillMatchResult result;
	private List<EbillMatchResult> resultList = new ArrayList<EbillMatchResult>();
	
	private IAccnoMainDataAdm accnoMainDataAdm;
	
	private static WFLogger logger = WFLogger.getLogger(EbillMatchReportBizImpl.class);
	@Override
	public List<EbillMatchResult> getEbillMatchReportResult(
		...
		try {
			...
			List<?> list = null;
			list = accnoMainDataAdm.getEbillMatchReportResult(queryMap,
					queryParam,docDate,isPaged,selectCount);
...
//			}
			...
			if (list != null && list.size() > 0) {
				if(selectCount!= null && selectCount.equals("countIdBank")){
					...
					for (int i = 0; i < list.size(); i++) {
						...
						result = new EbillMatchResult(idCenter,idBank, cName, SendCount1,
	...
	public static void setLogger(WFLogger logger) {
		EbillMatchReportBizImpl.logger = logger;
	}
