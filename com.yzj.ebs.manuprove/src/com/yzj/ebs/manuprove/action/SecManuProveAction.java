...
public class SecManuProveAction extends ActionSupport {
	/**
	 *
	 */
	private static final long serialVersionUID = 5479049316183701712L;
	/**
	 *
	 */

	private WFLogger logger = WFLogger.getLogger(SecManuProveAction.class);
	...
	public String init() {
		...
		try {
			...
			if (sealMode == null || sealMode == "") {
				sealMode = publicTools.getSysbaseParam("SEALMODE");
				if (sealMode == null) {
					sealMode = "0";
				}
			}
	...
	public String getTask() {
		XPeopleInfo userInfo = (XPeopleInfo) ServletActionContext
				...
				.getAttribute(AMSecurityDefine.XPEOPLEINFO);
		try {
			task = taskAdm.getTask(AppId.MANUALPROVE_SND, userInfo);
			if (task != null) {
	...
	public String getTaskByAjax()throws IOException{
		...
		try {
			...
			if (task != null) {
				...
				localManuProveJsonObj.setAccNo(localManuProveJsonObj.getAccNoList()
						.split("[|]")[0]);
	...
	public String submitTask() throws IOException {
		...
		} else if ("abandon".equals(operType)) {
		}
		try {
			insertSealLog();
