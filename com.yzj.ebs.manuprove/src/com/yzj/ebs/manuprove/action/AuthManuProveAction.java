...
public class AuthManuProveAction extends ActionSupport {

	private static final long serialVersionUID = -3133555577901679759L;
	private WFLogger logger = WFLogger.getLogger(AuthManuProveAction.class);
	...
	public String getTask() {
		XPeopleInfo userInfo = (XPeopleInfo) ServletActionContext
				...
				.getAttribute(AMSecurityDefine.XPEOPLEINFO);
		try {
			task = taskAdm.getTask(AppId.MANUALPROVE_AUTH, userInfo);
			...
			if (task != null) {
				...
				manuProveJsonObj.setAccNo(manuProveJsonObj.getAccNoList().split("[|]")[0]);
