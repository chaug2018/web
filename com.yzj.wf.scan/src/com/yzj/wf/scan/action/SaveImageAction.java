 ...
 * @version 1.0
 */
public class SaveImageAction implements IConfirmAction {

	private ScanToolBarPanel toolBarPanel;

	private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(SaveImageAction.class);
	private IUploadService upService;
	private static SaveImageAction instance=new SaveImageAction();
			...
			public void run() {
				...
				for (int i = 0; i < nodes.size();) { // 循环上传图像
					...
					for (int j = 0; j < list.size(); j++) {
						...
						if(ParamDefine.isDelete){
						ImageUtility.deleteFile(list.get(j).getFile());
						}
					}
					if(ParamDefine.isDelete){
					((DefaultTreeModel) tree.getModel()).removeNodeFromParent(curParent);
					}
					tree.repaint();
					finish++;
				}
//				upService.upload（null）;
				ShowProcessWindow.instance().stopProcessWindow();
