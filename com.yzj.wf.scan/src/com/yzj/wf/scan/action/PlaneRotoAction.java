 ...
 * @version 1.0.1
 */
public class PlaneRotoAction implements IConfirmAction {

	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(PlaneRotoAction.class);
	...
	public boolean confirm() {
		...
		if (storeContext.getSelectedImageNode() == null || storeContext.getSelectedImageNode().length == 0) {
			JOptionPane.showMessageDialog(null, "<html>请先选择需要<font color=red>水平翻转</font>的影像资料！</html>", "提示信息", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		...
		new Thread() {
			public void run() {
				ShowProcessWindow.instance().startProcessWindow("影像水平翻转中...");
			}
		}.start();
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					message = "影像水平翻转失败！失败原因：" + e.getMessage();
					log.error(message);
...
//			imageNode.setPicchangeFlag(0x04);
	...
	/**
	 * 水平翻转,如果temp_file不为空的话，保存修改后的图片 否则修改不在本地保存
	 * 
	 * @param imagebuffer
	 *            图片源
	 * @param temp_file
	...
	public void planeRotoImage(BufferedImage imagebuffer, File temp_file) throws Exception {
		imagebuffer = ImageUtility.planeRotoImage(imagebuffer, new AffineTransform(-1, 0, 0, 1, imagebuffer.getWidth() - 1, 0));
		// 保存图片
		if (temp_file != null)
