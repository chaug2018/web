 ...
 * @version 1.0.0
 */
public class BatchClockwiseRotateAction implements IConfirmAction {

	private ScanToolBarPanel toolBarPanel;

	private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(BatchClockwiseRotateAction.class);

	public BatchClockwiseRotateAction() {
	...
	public boolean confirm() {
		...
		if (paths == null || paths.length == 0) {
			logger.warn("没有选择需要正向旋转的影像");
			String temp = ParamDefine.rotateNotice_a;
			JOptionPane.showMessageDialog(toolBarPanel.getView(), temp, "提示信息", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		...
		new Thread("影像正向旋转进度条线程") {
			public void run() {
				ShowProcessWindow.instance().startProcessWindow(ParamDefine.rotating);
			}
		}.start();
		
		new Thread("影像正向旋转处理线程") {
			long curenttime = 0L;
			public void run() {
				try {
					curenttime = System.currentTimeMillis();
					RotoImage(paths);
					logger.info("影像正向旋转处理完成，系统耗时：" + (System.currentTimeMillis() - curenttime));
				} finally {
	...
	public void RotoImage(final TreePath[] paths) {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			if (node != null) {
				if (node.getLevel() == 1) { // 如果所选节点为父级节点，则操作其所有子节点
					BatchTreeNode[] childNodes = node.getChildNodes();
					for (BatchTreeNode childNode : childNodes) {
						rotoImage(childNode, bufferedImage);
					}
				} else {
