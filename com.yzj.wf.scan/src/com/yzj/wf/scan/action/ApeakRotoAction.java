 ...
 * @version 1.0.0
 * 
 * 类修改原因同<CODE>DregreeRotoAction</CODE>
 * @author LiuQiangQiang
 * @version 1.0.1
 */
public class ApeakRotoAction implements IConfirmAction {

	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ApeakRotoAction.class);
	...
	public boolean confirm() {
		ImageStoreContext context = toolBarPanel.getAppletPanel().getSysCacheContext().getImageStoreContext();
		// 核查是否选择多个图片
		if (context.getSelectedImageNode() == null || context.getSelectedImageNode().length == 0) {
			JOptionPane.showMessageDialog(null, "<html>请先选择需要<font color=red>垂直翻转</font>的影像资料！</html>", "提示信息", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}

		imageNodes = context.getSelectedImageNode();
		if (imageNodes == null || imageNodes.length == 0)
			return false;

		new Thread() {
			public void run() {
				ShowProcessWindow.instance().startProcessWindow("影像垂直翻转中...");
			}
		}.start();
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					message = "影像垂直翻转失败！失败原因：" + e.getMessage();
					log.error(message);
...
//			imageNode.setPicchangeFlag(0x04);
	...
	/**
	 * 垂直旋转，如果temp_file不为空的话，保存修改后的图片 否则修改不在本地保存
	 * 
	 * @param imagebuffer
	 *            图片源
	 * @param temp_file
	...
	public void planeRotoImage(BufferedImage imagebuffer, File temp_file) throws Exception {
		imagebuffer = ImageUtility.planeRotoImage(imagebuffer, new AffineTransform(1, 0, 0, -1, 0, imagebuffer.getHeight() - 1));
		// 保存图片
		if (temp_file != null)
