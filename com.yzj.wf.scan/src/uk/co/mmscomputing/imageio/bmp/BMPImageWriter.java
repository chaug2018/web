...
public class BMPImageWriter extends ImageWriter implements BMPConstants {
	...
	private int xPelsPerMeter = 2953; /*
										 * Horiz Pixels Per Meter, default 75
										 * dpi
										 */
	...
	int getHeaderSize() {
		...
		size += (compression == BI_BITFIELDS) ? 12 : 0; // or colour masks or
		// none of them
		return size;
	}

	private void writeHeader(ImageOutputStream out, BufferedImage image,
			IIOMetadata md) throws IOException {
	...
	public void write(ImageOutputStream out, BufferedImage image, IIOMetadata md)
			throws IOException {
		...
		try {
			...
			throw new IOException(getClass().getName()
					+ ".write : Couldn't grab pixels from image !");
		}
	...
	void writeColorTable(ImageOutputStream out, IndexColorModel icm)
			throws IOException {
	...
	void writeImage(ImageOutputStream out, byte[] buffer, int bytesPerLine)
			throws IOException {
	...
	private void write01bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md, IndexColorModel icm) throws IOException {
	...
	private void write04bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md, IndexColorModel icm) throws IOException {
	...
	private void write08bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md, IndexColorModel icm) throws IOException {
	...
	private void write16bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md) throws IOException {
	...
	private void write24bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md) throws IOException {
	...
	private void write32bit(ImageOutputStream out, BufferedImage image,
			IIOMetadata md) throws IOException {
	...
	public void write(IIOMetadata streamMetadata, IIOImage img,
		...
		if (cm instanceof IndexColorModel) {
			...
			case BufferedImage.TYPE_USHORT_555_RGB:
				write16bit(out, image, md);
				break;
			case BufferedImage.TYPE_3BYTE_BGR:
				write24bit(out, image, md);
				break;
			case BufferedImage.TYPE_INT_RGB:
				write32bit(out, image, md);
				break;
			default:
				write(out, image, md);
				break;
