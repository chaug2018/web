...
jobject BMP_transferImage(JNIEnv* env, jclass jclazz,jboolean* hasException, HGLOBAL dib){
  ...
  if(dib!=NULL){
    BITMAPINFOHEADER* bih=(BITMAPINFOHEADER*)GlobalLock(dib);           // get a pointer to the beginning of the DIB
    if(bih!=NULL){ 
...
void BMP_writeImage(JNIEnv* env, jclass jclazz,jboolean* hasException, HGLOBAL dib){
  ...
  if(dib!=NULL){
    len=GlobalSize(dib);                              
    bih=(BITMAPINFOHEADER*)GlobalLock(dib);           // get a pointer to the beginning of the DIB
    if(bih!=NULL){ 
