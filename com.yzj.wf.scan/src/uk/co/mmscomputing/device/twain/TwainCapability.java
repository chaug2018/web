...
*/
  ...
  protected int            cap;
  protected byte[]         buf=new byte[8];                       // TW_CAPABILITY
  ...
  private TwainContainer get(int msg,int contype)throws TwainIOException{
    ...
    jtwain.setINT32(buf,4,0);                                     // hContainer : source will set handle to container
    ...
    int    containerPtr  =jtwain.getINT32(buf,4);                 // ptr to native capability container
  ...
  private TwainContainer set(TwainContainer container)throws TwainIOException{    // use only in state 4
    ...
    int    containerHandle  =jtwain.nsetContainer(containerType,containerBytes);  // allocate native memory and copy container data into it.

    try{
      jtwain.setINT16(buf,0,cap);                                 // set capability
      jtwain.setINT16(buf,2,containerType);                       // set container type
      jtwain.setINT32(buf,4,containerHandle);                     // set native container

      source.call(DG_CONTROL,DAT_CAPABILITY,MSG_SET,buf);
    }catch(TwainResultException.CheckStatus trecs){
      container=get();
    }finally{
      jtwain.nfree(containerHandle);                              // release native memory again
    }
    return container;
  }
  ...
  static public TwainCapability[] getCapabilities(TwainSource source)throws TwainIOException{
    ...
    for(int i=0;i<items.length;i++){
      ...
      try{
        ...
        if(name==null){name="Cap_0x"+Integer.toString(capid);}
